...
def cal_9th_point(trajectory,time_step):
    ...
    x_next = x_last + vx_last * time_step + 0.5 * ax * time_step**2
    y_next = y_last + vy_last * time_step + 0.5 * ay * time_step**2
...
navi_path='/lpai/volumes/ad-e2e-vol-ga/zhengyupeng/navsim_exp/1_goal_point_unc/test_mlp/navi.npy'
...
navis=np.load(navi_path,allow_pickle=True).item()
result=copy.deepcopy(navis)
...
np.save('/lpai/socket/navsim_exp/1_goal_point_unc/navhard_default/navhard_mlp_recursion.npy',result)