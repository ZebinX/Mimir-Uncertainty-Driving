...
class AbstractAgent(torch.nn.Module, ABC):
    ...
    def forward(self, features: Dict[str, torch.Tensor],goalpoint=None) -> Dict[str, torch.Tensor]:
        """
    ...
    def compute_trajectory(self, agent_input: AgentInput,goalpoint=None,token=None) -> Tuple[np.ndarray,np.ndarray]:
        """
        ...
        # forward pass
        with torch.no_grad():
            predictions = self.forward(features,goalpoint=goalpoint,token=token)
